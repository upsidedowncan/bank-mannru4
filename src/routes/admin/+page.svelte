<script lang="ts">
	...
	async function rejectUser() {
		...
		isRejecting = true
		try {
			const { error } = await adminService.rejectUser(selectedApproval.id, user.id, rejectReason)
			if (!error) {
	...
	async function updateCard() {
		...
		isUpdatingCard = true
		try {
			const { error } = await adminService.adminUpdateCard(selectedCard.id, cardUpdates)
			if (!error) {
				showCardModal = false
				await loadCards()
			}
...
{:else if !isAdmin}
	<div class="min-h-screen bg-gray-50 flex items-center justify-center">
		<div class="text-center">
			<Icon icon="mdi:shield-alert" class="w-16 h-16 text-red-500 mx-auto mb-4" />
	...
	<div class="min-h-screen bg-gray-50">
			...
			<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div class="flex justify-between h-16">
					<div class="flex items-center">
						...
						<h1 class="text-2xl font-light text-primary tracking-tight">Админ панель</h1>
		...
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
						...
						<div class="space-y-4">
							{#each settings as setting}
								<div class="flex items-center justify-between">
									<div>
										<p class="font-medium text-gray-900">{setting.description}</p>
									...
									<input 
										...
										on:change={(e) => updateSetting(setting.setting_key, e.target.checked)}
									/>
								</div>
							{/each}
						</div>
			...
			{#if activeTab === 'settings'}
				<div class="bg-white rounded-lg shadow-sm p-6">
					<h2 class="text-xl font-light text-gray-900 mb-6">Настройки системы</h2>
					{#if loadingSettings}
								...
								<div class="flex items-start justify-between p-4 border border-gray-200 rounded-lg">
									<div class="flex-1">
										...
										<p class="text-sm text-gray-500 mb-2">
											Ключ: <code class="bg-gray-100 px-1 rounded">{setting.setting_key}</code>
										</p>
			...
			{#if activeTab === 'approvals'}
				<div class="bg-white rounded-lg shadow-sm">
					<div class="p-6 border-b border-gray-200">
						<h2 class="text-xl font-light text-gray-900">Заявки на регистрацию</h2>
						<p class="text-gray-600 mt-1">Управление заявками пользователей на регистрацию</p>
					</div>
					...
					{:else if pendingApprovals.length === 0}
						<div class="text-center py-12">
							<Icon icon="mdi:check-circle" class="w-12 h-12 text-green-500 mx-auto mb-4" />
								...
								<tbody>
									{#each pendingApprovals as approval}
										<tr>
											...
											<td>{new Date(approval.created_at).toLocaleDateString('ru-RU')}</td>
											<td>
												<div class="flex space-x-2">
													<button 
														on:click={() => approveUser(approval.id)}
										...
										<tr>
											...
											<td>
												{#if user.user_approvals?.[0]}
													<span class="badge {user.user_approvals[0].status === 'approved' ? 'badge-success' : user.user_approvals[0].status === 'rejected' ? 'badge-error' : 'badge-warning'}">
														{user.user_approvals[0].status === 'approved' ? 'Одобрен' : user.user_approvals[0].status === 'rejected' ? 'Отклонен' : 'Ожидает'}
													</span>
												{:else}
										...
										<tr>
											<td>
												<div>
													...
													<div class="text-sm text-gray-500">**** **** **** {card.card_number.slice(-4)}</div>
												</div>
											</td>
											<td>
												{#if card.user_profiles}
													<div class="font-medium">
														{card.user_profiles.first_name} {card.user_profiles.last_name}
													</div>
													<div class="text-sm text-gray-500">{card.user_profiles.email}</div>
												{:else}
													<span class="text-gray-400">—</span>
												{/if}
											</td>
											<td>
												<span class="badge {card.card_type === 'credit' ? 'badge-warning' : card.card_type === 'debit' ? 'badge-info' : 'badge-success'}">
													{card.card_type === 'credit' ? 'Кредитная' : card.card_type === 'debit' ? 'Дебетовая' : 'Сберегательная'}
												</span>
											</td>
											<td>
												<div class="font-medium">{formatCurrency(card.balance)}</div>
												{#if card.card_type === 'credit' && card.credit_limit}
													<div class="text-xs text-gray-500">Лимит: {formatCurrency(card.credit_limit)}</div>
												{/if}
											</td>
											<td>
												<span class="badge {getStatusBadge(card.status)}">
													{card.status === 'active' ? 'Активна' : card.status === 'blocked' ? 'Заблокирована' : 'Закрыта'}
												</span>
											</td>
											<td>
												<div class="flex space-x-2">
		...
		<div class="modal-container bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto p-6 animate-in zoom-in-95 duration-200 slide-in-from-bottom-4 m-4">
			...
			{#if selectedApproval}
				<div class="mb-4 p-3 bg-gray-50 rounded-lg">
					<p class="font-medium">{selectedApproval.first_name} {selectedApproval.last_name}</p>
			...
			<div class="form-control mb-6">
				<label class="label" for="reject-reason">
					<span class="label-text font-medium text-gray-700">Причина отклонения</span>
				</label>
				<textarea 
					id="reject-reason"
...
{#if showCardModal}
	<div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
		<div class="modal-container bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto p-6 animate-in zoom-in-95 duration-200 slide-in-from-bottom-4 m-4">
			...
			{#if selectedCard}
				<div class="mb-6 p-4 bg-gradient-to-r from-primary to-blue-600 text-white rounded-lg">
					<div class="font-medium">{selectedCard.card_name}</div>
					<div class="text-sm opacity-90">**** **** **** {selectedCard.card_number.slice(-4)}</div>
				</div>
					...
					{#if selectedCard.card_type === 'credit'}
						<div class="form-control">
							<label class="label" for="credit-limit">
								<span class="label-text font-medium text-gray-700">Кредитный лимит</span>
							</label>
							<input 
								id="credit-limit"
								type="number" 

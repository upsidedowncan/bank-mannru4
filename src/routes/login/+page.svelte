      ...
      if (data.user) {
        ...
        if (isInviteOnly) {
          const { data: approvalData } =
            await adminService.getUserApprovalStatus(data.user.id);
...
<div
  class="min-h-screen bg-gradient-to-br from-primary via-blue-600 to-indigo-700 flex overflow-x-hidden"
>
  <!-- Left Side - Branding -->
  <div
    class="hidden lg:flex lg:w-1/2 flex-col justify-center items-center text-white p-12"
  >
  ...
  <div
    class="w-full lg:w-1/2 bg-white flex flex-col justify-center p-8 lg:p-12 overflow-x-hidden"
  >
    <!-- Mobile Logo -->
    <div class="lg:hidden text-center mb-8">
      <h1 class="text-3xl font-light text-primary mb-2 tracking-tight">
        Маннру
      </h1>
    ...
    <div class="max-w-md mx-auto w-full max-w-full">
      <div class="mb-8">
        <h2 class="text-3xl font-light text-gray-900 mb-2 tracking-tight">
          Добро пожаловать
        </h2>
      ...
      <form on:submit|preventDefault={handleLogin} class="space-y-6">
        ...
        <div class="flex items-center justify-between">
          <label class="label cursor-pointer flex items-center space-x-2">
            ...
            />
            <span class="label-text text-sm text-gray-600 font-light"
              >Запомнить меня</span
            >
          </label>
          <a
      ...
      <div class="mt-8 text-center space-y-2">
        <div class="flex justify-center space-x-6 text-sm">
          <a
            href="/support"
            class="text-gray-500 hover:text-gray-700 font-light transition-colors"
            >Поддержка</a
          >
          <a
            href="/mobile-app"
